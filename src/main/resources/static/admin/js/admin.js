...
Index.Init = function () {
    // 初始化高与宽
    var win = $(window);
    var width = win.width() - 182 - 0 - 2-8;
    var height = win.height() - 64 - 2 - 33;
    var height_c = height - 29 -8;
    $('#layout_body_left').height(height);
	$('#layout_splitter').height(height);
    $('#layout_body_right').height(height).width(width);
    $('#menu').height(height - 3);
    $('#content-container').height(height_c);
    $('#loading').css('padding-top', height_c / 2);

    // 菜单切换效果
    var tits = $('.menu-titile');
...
function toggleLeft(){
	$("#layout_body_left").toggle();

	var win = $(window);
	if( $("#split_pane").attr("class") == 'split_pane' ){
		//alert("llllllllllllll");

		$("#split_pane").addClass('splitPane_un');
		
		var width = win.width() - 2-8;
		$('#layout_body_right').width(width);

	} else {
		$("#split_pane").removeClass('splitPane_un');

		var width = win.width()-182 - 2-8;
		$('#layout_body_right').width(width);
	}
}
...
function resetLayout()
{
    ...
    $('#layout_body_left').height(height);
	$('#layout_splitter').height(height);
	$('#layout_splitter').css("padding-top",height/2);
    $('#layout_body_right').height(height).width(width);
    $('#menu').height(height - 3);
    $('#content-container').height(height_c);
    $('#loading').css('padding-top', height_c / 2);
	console.log($("#layout_splitter").css("padding-top"));
}
    ...
    if (ListId) {
        ...
        for (var i = 1; i < tags.length; i += 1) {
            ...
            tags[i].onmouseout = function(){
                this.className = this.className.replace(" t3", "")
            }
        }
    }
}
